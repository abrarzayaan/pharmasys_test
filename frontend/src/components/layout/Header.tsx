        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-11">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary-600 hover:bg-primary-500 text-white font-head font-bold text-xs transition-colors"
            >
            ...
            {catMenuOpen && (
              <div className="absolute left-0 mt-2 w-60 bg-bg-card border border-bg-border rounded-2xl shadow-card p-2 z-50 space-y-1">
                {categories.map((cat) => (
                  <Link
                    key={cat.id}
                    to={`/products?category=${cat.id}`}
                    onClick={() => setCatMenuOpen(false)}
                    className="flex items-center justify-between px-3 py-2 rounded-xl text-xs text-content-secondary hover:text-content-primary hover:bg-bg-surface transition-colors"
                  >
                    <span>{cat.name}</span>
